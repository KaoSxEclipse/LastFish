    ...
    async def card(self, ctx, *, lookup):
        ...
        with open('cards.csv', 'r') as file:
            ...
            # If card not found -> Send Error embed.
                ...
                embed = discord.Embed(title="**Uh Oh!**", description="Something went wrong!", color=0xFF2027)
                embed.add_field(name="__Try__", value=f'•Make sure the Card Name is not in quotes.\n•Check your spelling\n•Make sure it is a valid card')
                embed.set_footer(text="Contact KaoSxEclipse#0111 if it still isn't working.")
                await ctx.send(embed=embed)

    @card.autocomplete('lookup')
    async def cardname_autocomplete(self, ctx, cardname: str, ) -> list[app_commands.Choice[str]]:
        return [app_commands.Choice(name=name, value=name) for name  in self.card_names if cardname.lower() in name.lower()]
