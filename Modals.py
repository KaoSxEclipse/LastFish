...
class PlayerModal(discord.ui.Modal, title='Enter Player Names'):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        if len(player_names) < 2:
            minemb = discord.Embed(title="Not Enough Players", description="You need atleast two player to play LastFish!", color=discord.Color.red())
            await interaction.response.send_message(embed=minemb, ephemeral=True)
            return
        if len(player_names) > 8:
            maxemb = discord.Embed(title="Too many players", description="The maximum amount of players is 8", color=discord.Color.red())
            await interaction.response.send_message(embed=maxemb, ephemeral=True)

        if any([not name.strip() for name in player_names]):
            blankname = discord.Embed(title="Name Error", description="Please make sure your format is Player 1, Player 2 ETC.", color=discord.Color.red())
            await interaction.response.send_message(embed=blankname, ephemeral=True)
            return


        first_player = random.choice(player_names)
        SelectionEmb = discord.Embed(title="Selected Player:", description="", color=6750182)
        SelectionEmb.add_field(name=f"__**{first_player}**__",value=f"Play will continue clockwise starting with {first_player}")
...
class PlayerSelect(commands.Cog):
    ...
    @app_commands.command(name="playerselect", description="Have the bot choose the starting player.")
...
class SuggestionModal(discord.ui.Modal, title="Bot Suggestion Form"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        SuggEmb = discord.Embed(title="Suggestion Sent", description="", color=discord.Color.green())
        SuggEmb.add_field(name="", value="Thank you for your suggestion! This may or may not be added but any ideas are appreciated!")
...
class Suggestion(commands.Cog):
    ...
    @app_commands.command(name="suggest",description="Send an idea what should be added to the bot!")
